...
set :scm,             :git
set :repository,      "gitolite@git.alabs.es:precajobs.git"
...
namespace :deploy do
  ...
  task :finalize_update, :except => { :no_release => true } do
    ...
    run <<-CMD
      ...
      ln -sf #{shared_path}/db/precajobs_production.sqlite3 #{latest_release}/db/precajobs_production.sqlite3
    CMD
  ...
  task :restart, :except => { :no_release => true } do
    run "kill -s USR2 `cat /tmp/unicorn.precajobs.pid`"
  end
  ...
  task :stop, :except => { :no_release => true } do
    run "kill -s QUIT `cat /tmp/unicorn.precajobs.pid`"
  end  
